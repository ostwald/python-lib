...
class WebCatNode:
	def __init__ (self, element):
		...
		self.url = linkElement.getAttribute ("HREF")
		label = XmlUtils.getText (linkElement)
		self.link = webcatUtils.WebCatLink ((self.url, label))
...
class WebCat (UserDict):
	...
	def getChildren (self, title):
		...
		if not node:
			msg = 'getChildren cant find starting node ("%s")' % title
			raise Exception, msg
	...
	def getComposites (self, node, composites=[]):
		...
		"""
		if not node:
			raise Exception, "getComposites got an empty starting node"
		
		# print 'getComposites: ' + node.title
		if node.title in self.processedTitles:
			# print "Duplicate Title!"
			return
		else:
			self.processedTitles.append (node.title)
		if not node:
...
def processCompositeRecords (url, collection, prefix="FOO"):
	...
	node = cat.getNode (collection)
	if not node:
		msg = "processCompositeRecords didn't get a root node for %s" % url
		raise Exception, msg
...
def compositeTester():
	url = "http://library.ucar.edu/uhtbin/cgisirsi/JCv3vLI8Es/SIRSI/238320015/503/6900"
	collection = "NCAR Technical Notes 81-100"
