...
To transform from command line:
	...
	use the stylesheet at ~/bin/xsl/remove-namespaces-and-transform.xsl along
	with the transform script like so:

		~/bin/transform ~/bin/xsl/remove-namespaces-and-transform.xsl 1995-NSES-v1.2.5.xml transformed/
"""
import os
import shutil
...
script = "/home/ostwald/bin/xsl/remove-namespaces-and-transform.xsl"
stdsBase = "/home/ostwald/asn-standards-docs"



def transformDir (dir):
	print "\nlocalizing " + dir
	...
	for filename in stds:
		...
		transform (topic, filename)
		
		
			
def transform (topic, std):
